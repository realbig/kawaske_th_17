<?php
/**
 * Add a TinyMCE button to create [video_with_form_overlay] Shortcodes
 *
 * @since   1.0.0
 ...
 * @return      void
 */
add_action( 'admin_init', 'add_kwaske_video_with_form_overlay_tinymce_filters' );
function add_kwaske_video_with_form_overlay_tinymce_filters() {
    
    if ( current_user_can( 'edit_posts' ) && current_user_can( 'edit_pages' ) ) {
        
        add_filter( 'mce_buttons', function( $buttons ) {
            array_push( $buttons, 'video_with_form_overlay_shortcode' );
            return $buttons;
        } );
        
        // Attach script to the button rather than enqueueing it
        add_filter( 'mce_external_plugins', function( $plugin_array ) {
            $plugin_array['video_with_form_overlay_shortcode_script'] = get_stylesheet_directory_uri() . '/dist/assets/js/tinymce/video-with-form-overlay-shortcode.js';
            return $plugin_array;
        } );
 ...
 */
add_filter( 'kwaske_tinymce_l10n', 'kwaske_video_with_form_overlay_tinymce_l10n' );
function kwaske_video_with_form_overlay_tinymce_l10n( $l10n ) {
    ...
    $l10n['video_with_form_overlay_shortcode'] = array(
        'tinymce_title' => _x( 'Add Video with Form Overlay', 'Video With Form Overlay Shortcode TinyMCE Button Text', 'kwaske_th_17' ),
        'video_id' => array(
            'label' => __( 'YouTube Video ID', 'kwaske_th_17' ),
        ),
		'form_id' => array(
